...
from src.cache import atomic_write_bytes, atomic_write_parquet, read_cache_validated
from src.config import DATA_CACHE, FRED_API_KEY
...
def materialize_one(series_id: str, fred: Fred, *, force_refresh: bool = False) -> Path:
    ...
    meta = {
        ...
        "cache_written_at": datetime.now(timezone.utc).replace(tzinfo=None).isoformat(),
    }
