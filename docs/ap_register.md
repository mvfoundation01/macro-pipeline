...
## AP-AUTH-49 (NEW; codified 2026-05-13; RESOLVED at L5b-G 2026-05-15) — Planning-only branch precommit infra hygiene

**Status**: RESOLVED at L5b-G (2026-05-15; cherry-pick of precommit infrastructure from `claude/layer-5-build` to `main` per Strategic disposition cycle).
...
**Surfaced**: L5-B Task A pre-flight commit on `claude/layer-5-build-plan` (commit chain: `9a25619` — required cherry-pick `f223eb3` of `0dc3e8d` from build branch before commit could land). Re-surfaced at documentation suite v2.0 commit cycle (2026-05-14) when committing `docs/build-plans/*.md` to `main` triggered the precommit hook but `main` lacked the validator scripts; Track A applied tactical workaround (untracked validator scripts at `scripts/precommit/` outside git tracking) to enable that single commit, with permanent fix queued as L5b-G.
...
**Resolution (L5b-G, 2026-05-15)**: Precommit infrastructure migrated from `claude/layer-5-build` to `main` via L5b-G sub-phase per Strategic disposition cycle (small standalone reviewer-driven sub-phase; AP-AUTH-53 8th instance; documentation-primary within AP-AUTH-54 envelope at 8 instances). Migration scope: nine files at `scripts/precommit/`:

- `validate_dual_grep_in_verification.py` (AP-AUTH-41 v6 enforcer)
- `validate_no_cumulative_arithmetic.py` (AP-AUTH-42 enforcer)
- `install_hook.py` (hook bootstrap script)
- `README.md` (precommit-infra documentation)
- `tests/dual_grep_should_fail.md` + `tests/dual_grep_should_pass.md` (AP-AUTH-41 self-test fixtures)
- `tests/no_cumulative_arithmetic_should_fail.md` + `tests/no_cumulative_arithmetic_should_pass.md` (AP-AUTH-42 self-test fixtures)
- `tests/run_hook_self_tests.py` (self-test harness)

`docs/ap_register.md` (this file) also migrated from `claude/layer-5-build` per Strategic disposition (institutional documentation is in-scope for L5b-G; the layer-5-build version was the post-L5b-E-migration state with AP-AUTH-50/51/52/53/54 entries already populated).

Future docs commits on `main` no longer require Track A's untracked-validators tactical workaround. The R5 push remediation cycle (2026-05-14) workaround pattern is now obsolete; superseded by this resolution.

**Cross-reference**: commit `f223eb3` (cherry-pick of precommit infra onto build-plan branch, original source); L5b-G ACCEPT commit on `main` (resolution).
