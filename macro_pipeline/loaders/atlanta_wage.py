...
def _read_atlanta_wage_raw() -> pd.Series:
    ...
    if not WAGE_PATH.exists():
        raise IndicatorNotFoundError(
            indicator_id=INDICATOR_ID, source="ATLANTA_FED_WAGE_XLSX",
            reason=f"source file missing: {WAGE_PATH}",
            context={"path": str(WAGE_PATH)},
        )
    try:
        ...
        raise IndicatorParseError(
            indicator_id=INDICATOR_ID, source="ATLANTA_FED_WAGE_XLSX",
            reason=f"failed to read sheet {WAGE_SHEET!r}: {exc}",
            original_exception=exc,
            context={"path": str(WAGE_PATH), "sheet": WAGE_SHEET},
        ) from exc
    ...
    if SOURCE_COL not in raw.columns:
        raise IndicatorParseError(
            indicator_id=INDICATOR_ID, source="ATLANTA_FED_WAGE_XLSX",
            reason=f"'{SOURCE_COL}' column missing, have {list(raw.columns)}",
            context={"sheet": WAGE_SHEET, "expected_column": SOURCE_COL},
        )
