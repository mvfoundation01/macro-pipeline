...
def _read_shiller_raw() -> pd.DataFrame:
    from macro_pipeline.exceptions import IndicatorNotFoundError, IndicatorParseError
    if not SHILLER_PATH.exists():
        raise IndicatorNotFoundError(
            indicator_id="SHILLER", source="SHILLER_XLS",
            reason=f"source file missing: {SHILLER_PATH}",
            context={"path": str(SHILLER_PATH)},
        )
    try:
        ...
        raise IndicatorParseError(
            indicator_id="SHILLER", source="SHILLER_XLS",
            reason=f"failed to read sheet {SHILLER_SHEET!r}: {exc}",
            original_exception=exc,
            context={"path": str(SHILLER_PATH), "sheet": SHILLER_SHEET},
        ) from exc
    ...
    *, force_refresh: bool = False, apply_pipeline: bool = True
    ...
    for indicator_id, col_idx, unit, lo, hi, role, use_for, description in SHILLER_SERIES_SPEC:
        if col_idx >= raw.shape[1]:
            from macro_pipeline.exceptions import IndicatorParseError
            raise IndicatorParseError(
                indicator_id=indicator_id, source="SHILLER_XLS",
                reason=(
                    f"column index {col_idx} out of range "
                    f"(file has {raw.shape[1]} columns); layout may have shifted"
                ),
                context={"file_columns": int(raw.shape[1])},
            )
