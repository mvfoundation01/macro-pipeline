    ...
    if use_cache:
        ...
        try:
            ...
            if spec.get("unofficial_yahoo") and parquet.exists():
                ...
                # Layer 1.5D.1: convert transport-level failures to typed
                # IndicatorNetworkError so call sites can branch on the
                # recoverable flag.
                from macro_pipeline.exceptions import IndicatorNetworkError
                raise IndicatorNetworkError(
                    indicator_id=indicator_id, source="YAHOO_FINANCE",
                    original_exception=exc,
                    context={"yahoo_ticker": yahoo_ticker},
                ) from exc
