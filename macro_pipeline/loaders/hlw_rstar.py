...
def _read_hlw_raw() -> pd.DataFrame:
    ...
    if not HLW_PATH.exists():
        raise IndicatorNotFoundError(
            indicator_id="HLW_RSTAR", source="NYFED_HLW_CURRENT_XLSX",
            reason=f"source file missing: {HLW_PATH}",
            context={"path": str(HLW_PATH)},
        )
    try:
        ...
        raise IndicatorParseError(
            indicator_id="HLW_RSTAR", source="NYFED_HLW_CURRENT_XLSX",
            reason=f"failed to read sheet {HLW_SHEET!r}: {exc}",
            original_exception=exc,
            context={"path": str(HLW_PATH), "sheet": HLW_SHEET},
        ) from exc
    ...
    *, force_refresh: bool = False, apply_pipeline: bool = True
    ...
    for indicator_id, col_idx, lo, hi, description in HLW_SERIES_SPEC:
        if col_idx >= raw.shape[1]:
            ...
            raise IndicatorParseError(
                indicator_id=indicator_id, source="NYFED_HLW_CURRENT_XLSX",
                reason=(
