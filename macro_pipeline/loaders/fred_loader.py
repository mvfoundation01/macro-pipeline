...
def _fetch_latest(fred: Fred, series_id: str) -> pd.Series:
    ...
    if s is None or len(s) == 0:
        # Re-raise as IndicatorNotFoundError once retries are exhausted
        # (Layer 1.5D.1). The retry decorator above keeps retrying ValueError,
        # so this still loops; on final failure tenacity reraises.
...
def _to_typed_fred_error(
    exc: BaseException, series_id: str, *, vintage: bool = False,
) -> IndicatorLoadError:  # noqa: F821 - forward reference
    """Convert a fredapi failure into a typed IndicatorLoadError."""
    ...
    msg = str(exc).lower()
    src = "FRED_ALFRED" if vintage else "FRED_API"

    # fredapi yields ValueError("FRED returned no data for ...") or similar
    # for non-existent / empty series. Treat that as NotFound.
    if isinstance(exc, ValueError) and ("no data" in msg or "no vintage" in msg
                                          or "does not exist" in msg
                                          or "bad request" in msg):
        return IndicatorNotFoundError(
            indicator_id=series_id, source=src,
            original_exception=exc, reason=str(exc),
        )
    if isinstance(exc, ValueError) and ("api_key" in msg or "auth" in msg):
        return IndicatorAuthError(
            indicator_id=series_id, source=src, original_exception=exc,
        )
    return from_request_exception(exc, indicator_id=series_id, source=src)
...
def load_fred_series(
    ...
    *,
    ...
    if use_cache:
        ...
        except (RetryError, ValueError, ConnectionError, TimeoutError) as exc:
            raise _to_typed_fred_error(exc, series_id, vintage=True) from exc
        df = df[df["realtime_start"] <= vintage_ts]
        if df.empty:
            from macro_pipeline.exceptions import IndicatorNotFoundError
            raise IndicatorNotFoundError(
                indicator_id=series_id, source="FRED_ALFRED",
                reason=(
                    f"no observations were known on or before "
                    f"{vintage_ts.date()}"
                ),
                context={"vintage_date": str(vintage_ts.date())},
            )
        ...
        try:
            ...
            raise _to_typed_fred_error(exc, series_id, vintage=False) from exc
        raw_series.name = series_id
...
def load_fred_all(
    *,
    ...
    df = pd.concat(results, axis=1, sort=True) if results else pd.DataFrame()
    df.index.name = "date"
    return df, metadata
